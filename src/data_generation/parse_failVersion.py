...
class Post:
	...
	def __init__(self, title, body, sents, typeId, accepted_answerId, answer_count, owner_userId, creation_date,
				 parentId, closed_date):
		self.title = title
		self.body = body
		self.sents = sents
		self.typeId = typeId
		...
		# for event, post in sorted(context, key=lambda x: x[1].attrib['Id']):
			if event == 'end' and post.tag == 'row':
				...
				try:
					...
					owner_userId = None
				creation_date = datetime.datetime.strptime(post.attrib['CreationDate'].split('.')[0],
														   "%Y-%m-%dT%H:%M:%S")
				try:
					closed_date = datetime.datetime.strptime(post.attrib['ClosedDate'].split('.')[0],
															 "%Y-%m-%dT%H:%M:%S")
				except:
					closed_date = None
				if postTypeId == 2:
